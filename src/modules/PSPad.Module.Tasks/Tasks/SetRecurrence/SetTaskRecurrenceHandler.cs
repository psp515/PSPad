...
public sealed class SetTaskRecurrenceHandler(IDocumentStore<TodoTask> store, IUnitOfWork work, IClock clock)
    : ICommandHandler<SetTaskRecurrence>
{
    public async Task<CommandResult> HandleAsync(SetTaskRecurrence command, CancellationToken ct)
    {
